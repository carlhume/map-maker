...
public class PerlinNoiseHexTileGenerator : MonoBehaviour
{
    ...
    private void createHexTilemap() {
        ...
        for( int x = 0; x < mapWidthInHexes; x++ ) {
            for( int y = 0; y < mapHeightInHexes; y++ ) {
                GameObject hexToPlaceGO = chooseHex( (float)x / (float)mapWidthInHexes, (float)y / (float)mapHeightInHexes );
    ...
    private GameObject chooseHex( float x, float y ) {
        float perlin = Mathf.PerlinNoise( x, y );
        Debug.Log( "X: " + x + " Y: " + y + " Perlin: " + perlin + " Tile: " + ( Mathf.FloorToInt( perlin * prefabs.Length ) ) );
        GameObject prefab = prefabs[ Mathf.FloorToInt( perlin * prefabs.Length ) ];
